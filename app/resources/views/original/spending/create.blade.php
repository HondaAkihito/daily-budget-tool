...
<section class="container">
    ...
    <form action="{{ route('spending.store') }}" method="post" class="mt-5 date_option">
        ...
        <div class="form-group">
            ...
            <input type="date" id="date" name="date" class="date_border form-control" value="{{ old('date') }}">
            <!-- バリデーション個別表示 -->
            @error('date')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="title" name="title" placeholder="予算のタイトルを入力" value="{{ old('title') }}">
            <!-- バリデーション個別表示 -->
            @error('title')
                <div class="text-danger"><span>{{ $message }}</span></div>
            @enderror
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="amount" name="amount" placeholder="支出金額を入力" value="{{ old('amount') }}">
            <!-- バリデーション個別表示 -->
            @error('amount')
                <div class="text-danger"><span>{{ $message }}</span></div>
            @enderror
